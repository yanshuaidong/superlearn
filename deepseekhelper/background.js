...
function startService() {
  ...
  startHeartbeat();
  console.log('[DeepSeek Helper] 服务已启动');
}
...
function stopService() {
  ...
  stopHeartbeat();
  currentTask = null;
  console.log('[DeepSeek Helper] 服务已停止');
}
...
async function doHeartbeat() {
  ...
  try {
    ...
    if (data.has_task) {
      console.log('[DeepSeek Helper] 收到新任务:', data.task_id);
      currentTask = {
        id: data.task_id,
...
async function executeTask(taskId, question) {
  ...
  try {
    ...
    if (tabs.length === 0) {
      // 没有找到 DeepSeek 页面，报告失败
      await reportTaskResult(taskId, '', false, '未找到 DeepSeek 页面，请先打开 chat.deepseek.com');
      currentTask = null;
      return;
    }
...
async function reportTaskResult(taskId, answer, success, message) {
  try {
    ...
    const data = await response.json();
    console.log('[DeepSeek Helper] 结果已报告:', data);
  } catch (error) {
    console.error('[DeepSeek Helper] 报告结果失败:', error);
  }
}
...
console.log('[DeepSeek Helper] Background service worker 已启动');
